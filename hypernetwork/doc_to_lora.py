...
class SHINEProcessor:
    ...
    def __init__(self, encoder, tokenizer, chunk_size: int = 512, overlap: int = 64):
        ...
        self.attention_selector = nn.MultiheadAttention(embed_dim=embed_dim, num_heads=8)
        # Move attention selector to same device as encoder
        device = next(encoder.parameters()).device if hasattr(encoder, 'parameters') else torch.device('cpu')
...
class DocToLoRA:
    ...
    def __init__(self, base_model: str, use_shine: bool = True, encoder_model: str = "sentence-transformers/all-MiniLM-L6-v2", default_rank: int = 16):
